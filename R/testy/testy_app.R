...
#       theme_minimal()
context("output$bp.a.usu - číselná hodnota pro MV")
mv <- agendy %>% filter(usu == "Ministerstvo vnitra") %>% nrow()
p.MV.hotovo <- agendy %>% filter(usu == "Ministerstvo vnitra") %>% 
  filter(ukonu >= 1) %>% 
  nrow()/mv
p.MV.hotovo <- round(p.MV.hotovo * 100)
test_that('% hotových agend pro MV je 83 %', {
  expect_equal(p.MV.hotovo, 83)
})
...
test_that('ag.seznam vrátí 9 výsledků', {
  expect_equal(nrow(ag.seznam), 9)
})
...
test_that('p.hotovo vrátí hodnotu 15', {
  expect_equal(p.hotovo, 15)
})